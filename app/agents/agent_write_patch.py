...
class PatchAgent:
    ...
    def __init__(
        ...
        self._eval_summary_with_call_chain_fixes = None

    def add_eval_summary_with_call_chain_fixes(self, _eval_summary_with_call_chain_fixes):
        self._eval_summary_with_call_chain_fixes = _eval_summary_with_call_chain_fixes
    ...
    def _construct_init_thread(self) -> MessageThread:
        ...
        if self.bug_locs:
            ...
            if self._eval_summary_with_call_chain_fixes is None:
                if config.is_initial_patch_given is True:
                    ...
                    THE golden git diff patch is given to you, please not this is the git diff, not the patch, ypur goal is to create the patch in the <original>..</original><modified>... format as specified, 
                    use this git diff as the template.
                    This is a very strict requirement.
                    The golden git diff that resolves the issue is:
                    <golden patch>
                    {init_patch}
                    </golden patch>
                    This git diff is 100% correct—guaranteed. Do not doubt it, but follow the instructions with this in mind.
                    """
            ...
            if self._eval_summary_with_call_chain_fixes is not None:
                thread.add_user(self._eval_summary_with_call_chain_fixes)
        else:
