...
from app.api import agent_proxy, agent_write_locations, agent_write_patch, agent_write_hypothesis, agent_write_pre_hypothesis, agent_select_hypothesis
...
class ProjectApiManager:
    ...
    def dispatch_intent(
        ...
        if (intent.func_name not in self.api_functions) and (
            intent.func_name not in ["get_class_full_snippet", "propose_locs", "write_hypothesis", "select_hypothesis"]  # HM: adding write_hypothesis and select_hypothesis functions
        ):
        ...
        try:
            ...
            if intent.func_name in ["write_patch", "propose_locs", "write_hypothesis", "select_hypothesis"]:    # HM: adding write_hypothesis and select_hypothesis functions
    ...
    def proxy_apis(self, text: str) -> tuple[str | None, str, list[MessageThread]]:
        ...
        debug_val = None
        if globals.debug and debug_val is not None:
            ...
            msg_thread.add_system("Debug agent_proxy / proxy_apis text")
            msg_thread.add_user(debug_val)
            summary = "The tool used a hardcoded message from DEBUG mode."
            return debug_val, summary, msg_thread
    ...
    # HM: adding function for writing hypothesis
    def write_hypothesis(
        self,
        message_thread: MessageThread,
        print_callback: Callable[[dict], None] | None = None,
    ) -> tuple[str, str, bool]:
        """Based on the current context, ask another agent to write a hypothesis explaining the root cause of the issue.
        When you think the current information is sufficient to write a hypothesis to guide patch generation, invoke this tool.
        The tool returns a root-cause explanation/hypothesis based on the current available information.
        """

        debug_val = None
        if debug_val is None and globals.debug_config is not None and os.path.isfile(globals.debug_config):
            with open(globals.debug_config, "r") as f:
                debug_config = json.load(f)
            if "hypothesis_val" in debug_config.keys():
                debug_val = debug_config["hypothesis_val"]

        if globals.debug and debug_val is not None:
            # save debug hypothesis in file: agent_hypothesis_raw_1.txt
            hypothesis_file = pjoin(self.output_dir, "agent_hypothesis_raw_1.txt")
            with open(hypothesis_file, "w") as f:
                f.write(debug_val)
            summary_msg = "agent_write_hypothesis replaced by hardcoded message from DEBUG mode."
            print_acr(
                debug_val,
                f"Hypothesis Generation from DEBUG mode",
                print_callback=print_callback,
            )
            return summary_msg, summary_msg, True

        tool_output = agent_write_hypothesis.run(
            message_thread,
            self.output_dir,
            self.task,
            retries=globals.hypothesis_cand,
            print_callback=print_callback,
        )
        summary = "The tool returned the hypothesis written by another agent."
        # The return status returned follows that returned by write_patch, so is just True here
        return tool_output, summary, True

    def write_pre_hypothesis(
        self,
        message_thread: MessageThread,
        print_callback: Callable[[dict], None] | None = None,
    ) -> tuple[str, str, bool]:
        """Based on the problem statement, ask another agent to write a hypothesis explaining the root cause of the issue.
        Invoke this tool before stratified code search when option pre_hypothesis is enabled.
        The tool returns a root-cause explanation/hypothesis based on the current available information.
        """

        debug_val = None
        if globals.debug and debug_val is not None:
            # save debug hypothesis in file: agent_pre_hypothesis_raw_1.txt
            hypothesis_file = pjoin(self.output_dir, "agent_pre_hypothesis_raw_1.txt")
            with open(hypothesis_file, "w") as f:
                f.write(debug_val)
            summary_msg = "agent_write_pre_hypothesis replaced by hardcoded message from DEBUG mode."
            return summary_msg, summary_msg, True

        tool_output = agent_write_pre_hypothesis.run(
            message_thread,
            self.output_dir,
            self.task,
            retries=1,
    ...
    # HM: adding function for selecting a hypothesis with self-reflection
    def select_hypothesis(
        self,
        message_thread: MessageThread,
        print_callback: Callable[[dict], None] | None = None,
    ) -> tuple[str, str, bool]:
        """Based on the current context, ask another agent to select a hypothesis explaining the root cause of the issue from candidate hypotheses.
        """
        tool_output = agent_select_hypothesis.run(
            message_thread,
            self.output_dir,
            self.task,
            retries=1,
        ...
        summary = "The tool selected a hypothesis written by another agent."
        # The return status returned follows that returned by write_patch, so is just True here
        return tool_output, summary, True