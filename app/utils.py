...
def run_script_in_conda(
    args: list[str], env_name: str, shell: bool = False, **kwargs
) -> subprocess.CompletedProcess:
    if shell:
        # Join args into a single string for shell execution
        cmd = f"conda run -n {env_name} {' '.join(args)}"
...
def run_string_cmd_in_conda(
    ...
    #conda_cmd = f"source {conda_script_path} ; conda activate {env_name} ; {command} ; conda deactivate" ## Changed for non root user
...
def does_file_exist(file_path: str):
    ...
    Args:
        file_path (str): Name of the file you want to check.
    """
...
def extract_text_in_angle_brackets(angle_bracket_heading: str, content: str, single_string: bool = False):
    start = f"<{angle_bracket_heading}>"
    end = f"</{angle_bracket_heading}>"
    pattern = re.compile(f"{start}(.*?){end}", re.DOTALL)
    matches = pattern.findall(content)
    if not matches:
        return "" if single_string else []
    if single_string:
        return matches[0]
    else:
        return matches
    
def read_json_from_a_file(file_name: str):
    read_json = None
    with open(file_name, 'r') as f:
        read_json = json.load(f)
    return read_json

def read_file_and_create_map_between_issue_and_patch(file_name: str)->dict[str, str]:
    read_json = None
    issue_patch_map = {}
    with open(file_name, 'r') as f:
        read_json = json.load(f)
    for patch_result_json in read_json:
        issue_id = patch_result_json["instance_id"]
        patch = patch_result_json["model_patch"]
        issue_patch_map[issue_id] = patch
    return issue_patch_map    